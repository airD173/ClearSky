...
def main(zipcode):
    ...
    alertdata = requests.get('https://api.weather.gov/alerts/active?zone=' + requests.get(coord['properties']['county']).json()['properties']['id']).json()
    forecast = requests.get(coord['properties']['forecast']).json()['properties']['periods']
    
    if len(alertdata['features']) == 0:
        alerts = None
    else:
